...
import { AlertTriangle, MapPin, Clock, X, CheckCircle, ChevronDown } from "lucide-react";

const ALERTS = [
  {
    id: "AMB-2401", name: "John Doe", age: 52, gender: "M",
    severity: "critical", condition: "Cardiac Arrest", eta: 4,
    vitals: { hr: 145, bp: "180/110", spo2: 88, temp: 38.2 },
    room: "Trauma Bay 2", unit: "Unit 23-Alpha",
    notes: "CPR in progress, defibrillated x2, IV access established.",
    distance: "2.3 km", team: "Cardiology",
  },
  {
    id: "AMB-2402", name: "Jane Smith", age: 34, gender: "F",
    severity: "urgent", condition: "Motor Vehicle Accident", eta: 8,
    vitals: { hr: 98, bp: "130/85", spo2: 95, temp: 37.1 },
    room: "ER-5", unit: "Unit 15-Bravo",
    notes: "Multiple contusions, possible rib fracture. Alert and oriented.",
    distance: "5.8 km", team: "Trauma",
  },
];

const SEV = {
  critical: { bar: "bg-red-600",    badge: "bg-red-600 text-white",    ring: "ring-red-400",    bg: "bg-red-50",    border: "border-red-200",    label: "CRITICAL" },
  urgent:   { bar: "bg-orange-500", badge: "bg-orange-500 text-white", ring: "ring-orange-400", bg: "bg-orange-50", border: "border-orange-200", label: "URGENT"   },
  moderate: { bar: "bg-yellow-500", badge: "bg-yellow-500 text-white", ring: "ring-yellow-400", bg: "bg-yellow-50", border: "border-yellow-200", label: "MODERATE" },
};
...
export default function IncomingAlerts() {
  ...
  const pageTeam = async (team, patient) => {
    setPaging(true);
    await new Promise(r => setTimeout(r, 900));
    toast(`${team} paged for ${patient.name}`);
    setPaging(false);
  };

  const confirm = (a) => { toast(`${a.name} confirmed → ${a.room}`); setModal(null); };
  const reassign = (a) => {
    const r = prompt("New room:", a.room);
    if (r) toast(`${a.name} reassigned to ${r}`);
  };
    ...
    <div className="max-w-screen-xl mx-auto space-y-5">
      ...
      {notif && (
        <div className={`fixed top-20 right-5 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-2xl text-white min-w-64
          ...
          <button onClick={() => setNotif(null)}><X className="w-4 h-4 opacity-70 hover:opacity-100" /></button>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {filtered.map(a => {
            ...
            return (
              <div key={a.id}
                className={`bg-white rounded-xl border-2 ${s.border} shadow-sm overflow-hidden
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">{a.age}y • {a.gender} • {a.id}</p>
                  ...
                  <div className="grid grid-cols-4 gap-2 mb-4">
                    {[
                      { l: "HR",    v: `${a.vitals.hr}`,       u: "bpm", warn: a.vitals.hr > 100 },
                      { l: "BP",    v: a.vitals.bp,             u: "",    warn: false              },
                      { l: "SpO2",  v: `${a.vitals.spo2}%`,    u: "",    warn: a.vitals.spo2 < 92 },
                      { l: "Temp",  v: `${a.vitals.temp}°C`,   u: "",    warn: a.vitals.temp > 38 },
                    ].map(v => (
                      <div key={v.l} className={`rounded-lg p-2.5 text-center border
                        ${v.warn ? "bg-red-50 border-red-200" : "bg-slate-50 border-slate-200"}`}>
                        <p className="text-xs text-slate-400 mb-0.5">{v.l}</p>
                        <p className={`text-sm font-black ${v.warn ? "text-red-600" : "text-slate-800"}`}>{v.v}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 text-xs text-slate-500">
                      <span className="flex items-center gap-1"><MapPin className="w-3 h-3" />{a.unit}</span>
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{a.distance}</span>
                    </div>
                    <div className="flex items-center gap-2">
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto"
            ...
            {/* Modal header */}
            <div className={`${SEV[modal.severity].bar} h-2 rounded-t-2xl`} />
            <div className="p-6">
              <div className="flex justify-between items-start mb-5">
                <div>
                  <h3 className="text-xl font-bold text-slate-800">{modal.name}</h3>
                  <p className="text-sm text-slate-500">{modal.id} • {modal.age}y • {modal.gender} • {modal.condition}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-5">
                {[
                  ...
                  <div key={l} className={`flex justify-between items-center p-2.5 rounded-lg border
                    ${warn ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}>
                    <span className="text-xs text-slate-500">{l}</span>
                    <span className={`text-sm font-bold ${warn ? "text-red-600" : "text-green-700"}`}>{v}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-5">
                ...
                <button onClick={() => pageTeam(modal.team, modal)} disabled={paging}
                  className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white
                    text-sm py-2 rounded-lg font-bold transition-colors">
